...
mod amm {
    ...
    impl Amm {
        ...
        pub fn faucet(&mut self, _amountToken1: Balance, _amountToken2: Balance) {
            let caller = self.env().caller();
            let token1 = self.token1Balance.get(&caller).unwrap_or_default();
            let token2 = self.token2Balance.get(&caller).unwrap_or_default();
        ...
        pub fn getMyHoldings(&self) -> (Balance, Balance, Balance) {
            ...
            let token1 = self.token1Balance.get(&caller).unwrap_or_default();
            let token2 = self.token2Balance.get(&caller).unwrap_or_default();
            let myShares = self.shares.get(&caller).unwrap_or_default();
        ...
        pub fn swapToken2GivenToken2(
            ...
            let newToken1Balance = self
                ...
                .map(|val| val + amountToken1)
                .unwrap();
            self.token2Balance.insert(caller, &newToken1Balance);
            Ok(amountToken1)
        }
